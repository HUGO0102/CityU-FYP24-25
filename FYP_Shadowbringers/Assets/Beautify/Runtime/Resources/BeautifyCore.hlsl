...
#define __BEAUTIFY_CORE
    ...
    float4 ApplyBeautify(float2 uv) : SV_Target
    {
		...
		float3 rgbS       = SAMPLE_TEXTURE2D_X(_SourceTex, sampler_LinearClamp, scaledUVS).rgb;
	    float3 rgbW       = SAMPLE_TEXTURE2D_X(_SourceTex, sampler_LinearClamp, scaledUVW).rgb;
	    float3 rgbE       = SAMPLE_TEXTURE2D_X(_SourceTex, sampler_LinearClamp, scaledUVE).rgb;
	    float3 rgbN       = SAMPLE_TEXTURE2D_X(_SourceTex, sampler_LinearClamp, scaledUVN).rgb;
    ...
    float4 FragCompare (Varyings input) : SV_Target {
        ...
        float4 pixel  = SAMPLE_TEXTURE2D_X(_SourceTex, sampler_LinearClamp, pixelUV * _InputScale.xy);
        float4 pixelNice = SAMPLE_TEXTURE2D_X(_CompareTex, sampler_LinearClamp, pixelNiceUV);
